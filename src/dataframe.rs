...
//! `impl` per engine, default `backend::native`) plus a backend-agnostic column-
...
pub fn read_csv(path: &str, line: usize, col: usize) -> Result<crate::backend::Df, crate::error::HelixError> {
    if crate::backend::polars_selected() {
        return crate::backend::polars::read_csv(path, line, col);
    }
    crate::backend::native::read_csv(path, line, col)
}

#[cfg(all(feature = "dataframes", feature = "native-df"))]
pub fn read_parquet(path: &str, line: usize, col: usize) -> Result<crate::backend::Df, crate::error::HelixError> {
    if crate::backend::polars_selected() {
        return crate::backend::polars::read_parquet(path, line, col);
    }
    crate::backend::native::read_parquet(path, line, col)
}
