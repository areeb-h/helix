# Memory safety & leak-freedom

Helix is written in safe Rust, and the interpreter is **provably leak-free**. This
note records the argument and the tests that back it.
...
The **one** `unsafe` in the codebase is `jit::call_native` — calling
Cranelift-generated machine code is inherently unsafe. It is confined to a single
...
module outlives every call. Everything else remains zero-`unsafe`, zero
`forget`/`leak`/`into_raw`/`ManuallyDrop`.

The first kind is **structurally impossible** here. Building an `Rc` cycle
requires *interior mutability* (a `RefCell`/`Cell`/`Mutex` to install a
...
`Function`, `Tensor`, `DataFrame`, …) only ever hold `Rc`s to values that already
exist, and collections are immutable (`Rc<Vec<Value>>`, never
...
Two tests in `src/interp.rs` back this empirically:
...
- **`no_reference_leaks`** — the test helper drops the `Interp` *before* returning
  ...
  `let`, destructuring, records, tuples, interpolation, `zip`+param-destructure,
  slicing). No `unsafe`, no external tools — `strong_count` is the ground truth.

- **Empirical RSS stability** (manual / `scripts`): running 10 vs **200**
  allocation-heavy operations (each building ~200k-element arrays and discarding
  them) holds peak RSS **flat at ~10 MB**. A real leak would grow RSS ~20×.

## Robustness: recursion depth

The interpreter is a tree-walker, so each Helix function call recurses on the
native stack (~25 KB/call in debug). Two measures keep this safe:

1. **`main` runs the interpreter on a dedicated 2 GiB-stack thread**, so deep but
   bounded recursion (e.g. `sum(15000)`) works instead of overflowing the default
   8 MB main stack.
2. **`MAX_CALL_DEPTH` (20 000)** turns runaway/too-deep recursion into a clean
   Helix error — *"maximum recursion depth exceeded"* — well before the 2 GiB
   stack could be exhausted, instead of an uncatchable stack-overflow abort.
