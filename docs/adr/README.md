...
| [0033](0033-native-dataframe-engine.md) | A native DataFrame engine — replace polars, staged, with polars as the oracle | **Accepted — Stages 0+1+2 implemented** (frozen format; native engine with differential parity; native parquet cross-compatible with polars; appliance does full frame IO at 11.3 MB); Stage 3 (parallel CSV) open |
