...
- **Status:** 🛠 v1 implemented (`src/python.rs`, behind the `python` feature)
- **Date:** 2026-06-24
...
Biopython, pysam, pandas, scikit-learn, PyTorch. Rebuilding all of that natively is
...
only where a library is needed, so Helix only has to be *better at its chosen core
workloads* rather than reimplement the world first. It does **not** make Helix a
better language — it makes Helix *usable before its own ecosystem exists*.
...
note). The guiding frame, consistent with Helix's philosophy: **an escape hatch
...
Embedding CPython means linking libpython, so a Python-enabled Helix is **not** a
self-contained binary. To protect Helix's "fast, safe, self-contained" value prop,
all interop lives behind an **off-by-default Cargo feature** (`python`); `pyo3` is
an optional dependency. The default `helix` never links libpython. Without the
feature, the `python` value still exists and `python.import` still parses, but
calling it returns a clean *"Helix was built without Python support — rebuild with
`--features python`"* error.

*Rejected:* always-on interop (taxes every user with a libpython dependency);
...
and the engines **never reference pyo3**. This is GraalPy's "single lazy `foreign`
handle" idea and Mojo's "one `PythonObject`" — keep Python values opaque and live,
don't eagerly model them in Helix's type system.

*Rejected:* multiple Python-typed variants (more ripple through every exhaustive
`Value` match); leaking `pyo3` types into `value.rs` (would force the whole crate to
depend on pyo3).

### D3 — Opaque-by-default conversion (the load-bearing decision)

This is the single most important call, and the one the research most sharply
...
destroys identity + mutability** (a Python `list` copied into a native array loses
...
including scalars (PythonCall's strict default — users found it heavy: `print(m.sqrt(16))`
shouldn't need a conversion call).

### D4 — Two surface syntaxes, one runtime path

Both forms the user asked for are supported, and they converge:
- `import python.math as m` — the statement form (reuses the dotted-path + `as`
  module syntax shipped just before this).
...
runtime path and the statement form is pure sugar.
...
hits the keyword). The postfix `.member` parser now accepts any reserved word as a
member name (`member_name()` beside `ident_name()`). This is a common, safe rule —
after `.`, a keyword can only be a member — and it incidentally enables `x.in`,
`obj.type`, etc.

*Rejected:* renaming the method (e.g. `python.load`) to dodge the clash (the
research and the user both specified `python.import`); making `import` contextual in
the lexer (more invasive, and the member rule is generally useful).
...
the GIL (pyo3 handles the attach). Helix's **deterministic `Rc`** sidesteps
PythonCall's two-GC leak, where a tracing GC couldn't "see" large Python objects
behind small wrappers and let memory balloon.

### D7 — Errors render-and-abort (not catch) for v1

A Python exception becomes a Helix-native diagnostic — `python error: <ExcType>:
<message>` at the call-site `line:col`, via `PyErr::get_type().name()` +
`value().str()`. Because Helix has **no `try`/`catch` yet**, v1 **aborts**;
recoverable Python errors wait for a Helix error-handling story (ADR 0004).

### D8 — Embedding idiom is free-threading-correct from day one

The bridge uses `Python::with_gil`/`attach` + the modern `Bound<'py,T>` API, and is
written to `detach()` around blocking work — so it is already correct for
free-threaded CPython (PEP 703, supported in 3.14), even though v1 is
single-threaded. `pyo3` features: `auto-initialize` + `abi3-py38` (one build spans
...
Mojo's #1 shipped failure is a runtime *"can't locate libpython."* So: the v1 spike
links the **ambient** Python (the user `pip install`s their own deps) but **fails
loudly at startup**, never cryptically mid-program. The committed end-state is to
**bundle a relocatable CPython** (python-build-standalone / PyOxidizer `pyembed`,
`$ORIGIN`-relative `sys.path`) so the feature build ships self-contained, with the
Python environment pinned in Helix's future lockfile.

*Rejected:* depend on a system Python and just document it (Mojo's documented worst
failure mode); bundle from day one (significant packaging work before the first
working demo — deferred, not abandoned).

## How `Unknown` makes the type checker a no-op

Helix's permissive type system already has the dynamic boundary interop needs:
`Unknown` is compatible-with-everything and propagates through every
method/field/index (it's how DataFrame columns are typed). `python` is seeded as
`Type::Unknown`, so `python.import("numpy").mean([...]).reshape(...)` type-checks
with **zero checker changes** — Python values ride the same escape hatch as
DataFrame columns, while native Helix code stays statically checked.
...
1. **Zero-copy scientific bridge (the differentiator).**
   - **DataFrame ↔ Python polars — shipped.** A Helix DataFrame crosses to/from
     ...
     unifies on our polars 0.54 + pyo3 0.28). `to_py` collects the LazyFrame and
   ...
   - **Tensor ↔ Python NumPy — shipped.** A Helix Tensor crosses to/from NumPy
     ...
     `<=0.17`, our 0.17). `to_py` copies the ndarray into a NumPy array; the
     ...
     Helix tensors are immutable + `Rc`-shared, so aliasing would break the
     immutability guarantee — each side gets its own buffer. Round-trip verified on
     both engines.
   - **Pending:** truly buffer-sharing tensors via **DLPack** (GPU/large arrays where
     mutability allows); pandas/pyarrow via the Arrow C stream interface; chunked
     Series via the stream interface (avoid a rechunk copy). Footguns to honor:
     DLPack consume-once (`"used_dltensor"` rename) leak; null loss through
     DLPack/NumPy; export immutable `Arc`-snapshots; GIL in release callbacks. The
     DataFrame + Tensor bridges are what Mojo can't do well and Helix's Polars/Arrow/
     ndarray backing makes natural.
2. **Bundled CPython** (python-build-standalone + `pyembed`) → self-contained
   feature build; Python env in the lockfile (ties into the package-manager work).
...
5. **Free-threading / sub-interpreters** — revisit once the bio-relevant native
   stack (NumPy et al.) is verifiably free-threaded / sub-interpreter-safe.
