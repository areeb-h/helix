...
1. **Recursion depth**: walker `MAX_CALL_DEPTH=20_000` vs VM `VM_MAX_DEPTH=1_000_000` — `s(50000)` errors on one engine, prints on the other two. And **infinite tail recursion**: walker errors at 20k; VM/JIT loop forever at constant memory (TCO is the deliberate B1 semantics — `while true` equivalence). Options: document engine-dependent budgets as sanctioned, or align (walker TCO + shared constant).
2. **Nested `missing` equality** (`ops.rs values_equal` hits `_ => false` for Missing/Missing): `{a: missing} == {a: missing}` → `false`, `(try 5) == (try 5)` → `false`, `[missing, missing].unique()` keeps both. Top-level `missing == missing` correctly yields `missing` — the nested case is internally inconsistent and diverges from ADR-0001's Julia/SQL model (needs three-valued structural equality + an identity-equality for unique/frequencies).
3. **Duplicate record-literal fields**: `{a: 1, a: 2}` keeps both — `x == y` is order-independent-true while `x.a ≠ y.a`, and `unique()` collapses them. `values_equal`'s record arm *assumes* no duplicate keys. Reject at parse/check time (spread-update already dedups last-wins).
4. **Tuple ordering**: `(1,2) < (1,3)` errors ("needs numbers") — either lexicographic ordering or a hint pointing at `sort_by(t => t[0])`.
